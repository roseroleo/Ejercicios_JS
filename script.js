...
let a = (frase) => frase.reverse().join("");
console.log(a(frase));
*/
...
const vA = [];
const vB = [];
function compararDosNumeros(a, b){
    if (a > b) return 1;
    if (a == b) return 0;
    if (a < b) return -1;
}
...
for (let i = 0; i < 50; i++) {
    vA[i] = parseInt(Math.random()*10);
    
}
console.log(JSON.stringify(vA));
vA.sort(compararDosNumeros);
...
const matriz = JSON.stringify(Persona);
console.log(matriz);
